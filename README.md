...
| **+ v2.18** | **2026-09-14** | **传动件原语：make_gear 真渐开线单闭合齿廓（Spline 齿面）+ 斜齿 twist-loft（β 可配）+ create_keyway（GB/T 1096，槽深实测）+ create_spline 花键；5 挡变速器 26 件总成实测** | **432** | **36+10** |
| **+ v2.19** | **2026-09-15** | **材质系统 materials.py（铸铁/钢/深钢/青铜…基色+高光+名称推断）+ 软件光栅渲染器 software_renderer.py（numpy z-buffer / Blinn-Phong / 2×SSAA / 自动取景）——无 GPU 也能出分材质证据图** | **441** | **36+10** |
